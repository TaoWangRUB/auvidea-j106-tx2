/* stm32h7xx_it.c — exception and interrupt handlers.
 *
...
/* ------------------------------------------------------------------ *
 * TIM5 update — one interrupt per emitted frame trigger.
 *
...
{
	if (htim5.Instance->SR & TIM_SR_UIF) {
...
void USART1_IRQHandler(void)
{
	...
	while (__HAL_UART_GET_FLAG(&huart1, UART_FLAG_RXNE))
		cli_rx_from_isr(SINK_UART,
				(char)(huart1.Instance->RDR & 0xFF), &woken);
