...
extern uint32_t g_lcd_w, g_lcd_h;
 ...
 * the timer clock ran.  TIM5's 32-bit counter removes that entirely - div is 1
...
static int tim1_program(void)
{
	...
	/* TIM5's counter is 32-bit, so the prescaler is only needed for periods
	...
	__HAL_TIM_SET_PRESCALER(&htim5, div - 1);
	...
	for (i = 0; i < NCH; i++) {
		...
		if (ccr > arr)
			return 0;
		__HAL_TIM_SET_COMPARE(&htim5, ch_of[i], g_running ? ccr : 0);
	}

	htim5.Instance->EGR = TIM_EGR_UG;	/* latch PSC/ARR/CCRx immediately */
	htim5.Instance->SR  = 0;
	return 1;
}
...
static void trig_start(void)
{
	...
	htim5.Instance->SR = 0;
	for (i = 0; i < NCH; i++)
		HAL_TIM_PWM_Start(&htim5, ch_of[i]);
	g_pwm_started = 1;
}
...
static void trig_stop(void)
{
	...
	for (i = 0; i < NCH; i++)
		__HAL_TIM_SET_COMPARE(&htim5, ch_of[i], 0);
	htim5.Instance->EGR = TIM_EGR_UG;	/* park idle before the timer halts */
	if (g_pwm_started) {
		for (i = 0; i < NCH; i++)
			HAL_TIM_PWM_Stop(&htim5, ch_of[i]);
		g_pwm_started = 0;
	}
	g_running = 0;
	g_burst_left = 0;
	HAL_GPIO_WritePin(GPIOE, 1U << LED_PIN, GPIO_PIN_SET);	/* LED off */
}
...
static void cmd_status(sink_t s)
{
	...
	for (i = 0; i < NCH; i++) {
		...
		out_puts(s, " ccr=");
		out_putu(s, __HAL_TIM_GET_COMPARE(&htim5, ch_of[i]));
		out_putc(s, '\n');
	}
	out_kv(s, "psc", htim5.Instance->PSC);
	out_kv(s, "arr", htim5.Instance->ARR);
	...
	out_kv(s, "lcd_w", g_lcd_w);
	out_kv(s, "lcd_h", g_lcd_h);
	out_kv(s, "backlight_pct", g_backlight);
	out_puts(s, "ok\n");
}
...
void camtrig_handle(char *line, sink_t s)
{
	...
	} else if (str_eq(line, "bl")) {
		uint32_t v = parse_u32(arg);

		if (v == BAD_U32 || v > 100u) {
			out_puts(s, "err usage: bl <0-100>\n");
		} else {
			lcd_set_backlight(v);
			out_puts(s, "ok\n");
		}
...
void camtrig_task(void *arg)
{
	...
	for (;;) {
		...
		HAL_GPIO_WritePin(GPIOE, 1U << LED_PIN,
