...
/* Bring the panel up and leave it cleared and lit.
 *
 * Replaces WeAct's LCD_Test(), which was a demo: it drew a logo bitmap (260 kB
 * of image data we do not vendor), then blocked waiting on the KEY button.
 * Neither belongs in firmware whose job is to keep four cameras triggered. */
void LCD_Init(void)
{
