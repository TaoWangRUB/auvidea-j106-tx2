...
brightness. The trigger is therefore generated by a **WeAct MiniSTM32H7** (`TIM1_CH1` on `PE9`),
fanned out over **one shared net** to all four modules' `XTR+`.
...
Exposure moves out of the sensor: `t_exp = t_XTRIG_low + 14.26 µs`. So `set_exposure()` stops
writing `SHS1` while triggered (it still returns success — failing it would fail the stream — but
says once per stream that the value is ignored), and all four cameras necessarily share one
exposure. Max rate is **59.95 fps** (`tTGPD` = 1126 H = 16.6815 ms), not a firmware limit.

⚠ **`XTRIG` is a 1.8 V input with a 3.3 V absolute maximum.** Whether the breakout's `XTR+` pad is
that raw pin or something already buffered must be *measured* before anything is connected —
[`hw-trigger/WIRING.md`](hw-trigger/WIRING.md) §3 is a mandatory gate with both outcomes and the
divider values for the 1.8 V case.
...
- **Hardware trigger for the four IMX296** (§5 *Stage 8*). Everything on the software side is done
  ...
  - **Missing: the two wires** between the STM32H7 (`PE9`, header P2‑32) and the four modules'
    `XTR+`/`XTR−` pads — and, first, the ⚠ level-domain measurement in
    [`hw-trigger/WIRING.md`](hw-trigger/WIRING.md) §3.
